...
# =================== 0. Import data ====================
df = pd. read_excel (r'100.xlsx', sheet_name='Input')
rows = df.shape[0]
columns = df.shape[1]
random.seed(10)
...
# =========== Performance plot ===========
if (len(emergarr) == len(all_er_mean)):
    from matplotlib import pyplot as plt
    f, ax1 = plt.subplots()
    ax1.plot(all_emerg, all_er_mean)
    lns1 = ax1.plot(all_emerg, all_er_mean, color="black", label='ord')
    lns = lns1
    labs = [l.get_label() for l in lns]
    ax1.legend(lns, labs, loc=7, fontsize=12)
    plt.show()

if (len(update_count) == len(all_er_mean)):
    from matplotlib import pyplot as plt
    f, ax1 = plt.subplots()
    ax1.plot(update_count, all_er_mean)
